...
var analyser, freqByteData, audio, audioContext, source, gainNode, maxMagnitude, c, context;
...
function drawCirclesOne() {
	...
	for (var i = 0; i < settings.circlesOne.circles; i += 1) {
		...
		context.strokeStyle = 'hsla(' + settings.circlesOne.hue + ', 50%, 50%, ' + ((1 / settings.circlesOne.circles) + 0.1) + ')';
		context.fillStyle = 'hsla(' + settings.circlesOne.hue + ', 50%, 50%, ' + (1 / settings.circlesOne.circles) + ')';
		context.beginPath();
		context.arc((WIDTH / 2 - circleOffSetX) ,(HEIGHT / 2 - circleOffSetY), totals[i].strength, 0, Math.PI*2, true);
		context.stroke();
		context.fill();
	...
	context.fillStyle = 'hsla(' + settings.circlesOne.hue + ', 50%, 50%, ' + (0.03 * ratio) + ')';
	context.fillRect(0, 0, WIDTH, HEIGHT);
}
...
function drawCirclesTwo() {
	...
	for (var i = 0; i < settings.circlesTwo.circles; i += 1) {
		...
		context.strokeStyle = 'hsla(' + settings.circlesTwo.hue + ', 50%, 50%, ' + ((1 / settings.circlesTwo.circles) + 0.1) + ')';
		context.fillStyle = 'hsla(' + settings.circlesTwo.hue + ', 50%, 50%, ' + (1 / settings.circlesTwo.circles) + ')';
		context.beginPath();
		context.arc(mouse.x ,mouse.y, totals[i].strength, 0, Math.PI*2, true);
		context.stroke();
		context.fill();
	...
	ratio = totalMagnitude / maxMagnitude;
	context.fillStyle = 'hsla(' + settings.circlesTwo.hue + ', 50%, 50%, ' + (0.005 * ratio) + ')';
	context.fillRect(0, 0, WIDTH, HEIGHT);
}
...
function drawCirclesThree() {
	...
	for (var i = 0; i < settings.circlesThree.circles; i += 1) {
		...
		context.strokeStyle = 'hsla(' + settings.circlesThree.hue + ', 50%, 50%, ' + ((1 / settings.circlesThree.circles) + 0.15) + ')';
		context.fillStyle = 'hsla(' + settings.circlesThree.hue + ', 50%, 50%, ' + ((1 / settings.circlesThree.circles) + 0.05) + ')';
		context.beginPath();
		context.arc(circles[i].x ,circles[i].y, circles[i].strength, 0, Math.PI*2, true);
		context.stroke();
		context.fill();
	...
	ratio = totalMagnitude / maxMagnitude;
	context.fillStyle = 'hsla(' + settings.circlesThree.hue + ', 50%, 50%, ' + (0.04 * ratio) + ')';
	context.fillRect(0, 0, WIDTH, HEIGHT);
}
...
function drawSpikes() {
	...
	context.fillStyle = 'rgba(0, 0, 0, ' + settings.spikes.bgAlpha + ')';
	context.fillRect(0, 0, WIDTH, HEIGHT);
	context.lineWidth = settings.spikes.lineWidth;
	context.strokeStyle = 'hsla(' + settings.spikes.hue + ', 50%, 50%, 0.1)';
	context.beginPath();
	...
	for (var i = 0; i < 1024; i += samplesPerSample) {
		...
		for (j = 0; j < settings.spikes.samples; j++) {
			...
			context.moveTo(x, y);
			context.lineTo((x - side1), (y - side2));
		}
...
function drawBars() {
	...
	for (var i = 0; i <= 600; i += freqInc) {
		...
		context.fillStyle = 'hsla(' + hue + ', ' + (30 + (strength * 70)) + '%, 50%, ' + strength + ')';
		context.fillRect((trueIndex * barWidth), 0, barWidth, HEIGHT);
